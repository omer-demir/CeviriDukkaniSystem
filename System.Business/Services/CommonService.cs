...
namespace System.Business.Services {
    public class CommonService : ICommonService {
        ...
        public ServiceResult<MessageDto> GetMessage(int messageId) {
            var serviceResult = new ServiceResult<MessageDto>();
            try {
                var message = _ceviriDukkaniModel.Messages
                    .Include(a => a.FromUser)
                    .Include(a => a.ToUser)
                    .Include(a => a.FromCustomer)
                    .Include(a => a.ToCustomer)
                    .FirstOrDefault(w => w.Id == messageId);
                if (message == null) {
                    throw new DbOperationException(ExceptionCodes.NoRelatedData);
                }
        ...
        public ServiceResult<List<CityDto>> GetCitiesByCountryId(int countryId) {
            ...
            try {
                var cities = _ceviriDukkaniModel.Cities.Where(w => w.CountryId == countryId && w.Active).OrderBy(a => a.Name).ToList();
        ...
        public ServiceResult<List<DistrictDto>> GetDistrictByCityId(int cityId) {
            ...
            try {
                var districts = _ceviriDukkaniModel.Districts.Where(w => w.CityId == cityId && w.Active).OrderBy(a => a.Name).ToList();
        ...
        public ServiceResult<List<TranslationQualityDto>> GetTranslationQualities() {
            var serviceResult = new ServiceResult<List<TranslationQualityDto>>();
            try {
                var data = _ceviriDukkaniModel.TranslationQualities.Where(w => w.Active).ToList();
